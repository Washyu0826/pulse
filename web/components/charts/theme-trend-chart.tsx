...
export function ThemeTrendChart({ trend }: { trend: ThemeTrendPoint[] }) {
  ...
  // 每個主題一條序列（逐日數值）。
  const series = THEME_ORDER.map((t) => trend.map((d) => d[t] ?? 0));
  const areas = buildStackedAreas(series, H, { W, PAD });

  const totals = THEME_ORDER.map((t, i) => ({
    theme: t,
    total: series[i].reduce((a, b) => a + b, 0),
    opacity: BAND_OPACITY[i] ?? 0.16,
  }));
  ...
  return (
    <figure className="card">
      ...
      {grandTotal > 0 ? (
        ...
        >
          {areas.map((d, i) => (
            <path key={THEME_ORDER[i]} d={d} fill={`rgb(77 116 234 / ${BAND_OPACITY[i] ?? 0.16})`} />
          ))}
        </svg>
      ) : (
